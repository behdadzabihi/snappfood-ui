...
// node_modules/@vaadin/flow-frontend/vaadin-map/mapConnector.js
...
// node_modules/@vaadin/flow-frontend/vaadin-map/synchronization/index.js
import Feature from "ol/Feature";
import Point2 from "ol/geom/Point";
import View from "ol/View";

// node_modules/@vaadin/flow-frontend/vaadin-map/synchronization/layers.js
import ImageLayer from "ol/layer/Image";
import TileLayer from "ol/layer/Tile";
import VectorLayer from "ol/layer/Vector";

// node_modules/@vaadin/flow-frontend/vaadin-map/synchronization/util.js
function convertToCoordinateArray(coordinate) {
  return [coordinate.x, coordinate.y];
}
function convertToSizeArray(size) {
  return [size.width, size.height];
}
function synchronizeCollection(collection, updatedIds, options) {
  const hasChanges = updatedIds.length !== collection.getLength() || collection.getArray().some((existingItem, index) => existingItem.id !== updatedIds[index]);
  if (!hasChanges)
    return;
  const updatedItems = updatedIds.map((id) => options.lookup.get(id)).filter((item) => !!item);
  collection.clear();
  updatedItems.forEach((item) => collection.push(item));
}
function createOptions(configurationObject) {
...
function synchronizeLayer(target, source, _context) {
  ...
  target.setZIndex(source.zIndex || void 0);
  target.setMinZoom(source.minZoom || -Infinity);
  target.setMaxZoom(source.maxZoom || Infinity);
  target.setBackground(source.background || void 0);
  return target;
}
function synchronizeTileLayer(target, source, context) {
  if (!target) {
...
function synchronizeVectorLayer(target, source, context) {
  if (!target) {
...
function synchronizeImageLayer(target, source, context) {
  if (!target) {
...
// node_modules/@vaadin/flow-frontend/vaadin-map/synchronization/sources.js
...
function synchronizeTileWMSSource(target, source, context) {
  if (!target) {
...
function synchronizeXYZSource(target, source, context) {
  if (!target) {
...
function synchronizeOSMSource(target, source, context) {
  if (!target) {
...
function synchronizeImageWMSSource(target, source, context) {
  if (!target) {
...
function synchronizeVectorSource(target, source, context) {
  if (!target) {
...
// node_modules/@vaadin/flow-frontend/vaadin-map/synchronization/styles.js
import Fill from "ol/style/Fill";
import Stroke from "ol/style/Stroke";
import Style from "ol/style/Style";
import { Icon } from "ol/style";
function synchronizeFill(target, source, context) {
  if (!target) {
    target = new Fill();
  }
...
function synchronizeStroke(target, source, context) {
  if (!target) {
    target = new Stroke();
  }
  target.setColor(source.color);
...
function synchronizeIcon(target, source, context) {
  if (!target) {
    ...
    target = new Icon(
      createOptions({
        ...
        img: void 0,
        src,
        imgSize: source.imgSize ? convertToSizeArray(source.imgSize) : void 0,
        anchor: source.anchor ? convertToCoordinateArray(source.anchor) : void 0,
        anchorOrigin: source.anchorOrigin ? convertAnchorOrigin(source.anchorOrigin) : void 0
      })
    );
  }
...
function synchronizeStyle(target, source, context) {
  ...
  target.setImage(
    source.image ? context.lookup.get(source.image) : void 0
  );
  target.setFill(
    source.fill ? context.lookup.get(source.fill) : void 0
  );
  target.setStroke(
    source.stroke ? context.lookup.get(source.stroke) : void 0
  );
  context.connector.forceRender();
  return target;
}
...
function synchronizePoint(target, source, _context) {
  if (!target) {
    target = new Point2(convertToCoordinateArray(source.coordinates));
  }
...
function synchronize(updatedObject, context) {
...
// node_modules/@vaadin/flow-frontend/vaadin-map/util.js
import VectorSource2 from "ol/source/Vector";
var SimpleLookup = class {
  constructor() {
...
var WeakReferenceLookup = class {
  ...
  get(id) {
    ...
    return weakRef ? weakRef.deref() : void 0;
  }
  put(id, instance) {
    if (this.map.has(id))
      return;
...
var supportsWeakReferenceLookup = window.WeakRef && window.FinalizationRegistry;
function createLookup() {
  return supportsWeakReferenceLookup ? new WeakReferenceLookup() : new SimpleLookup();
}
function getLayerForFeature(layers, feature) {
  return layers.find((layer) => {
    const source = layer.getSource && layer.getSource();
    const isVectorSource = source && source instanceof VectorSource2;
...
(function() {
    ...
    mapElement.$connector = {
      ...
      forceRender() {
        ...
        this._forceRenderTimeout = setTimeout(() => {
          ...
          mapElement.configuration.getLayers().getArray().forEach((layer) => layer.changed());
        });
      }
    };
  ...
  function defineProjection(projection, wksDefinition) {
    lib_default.defs(projection, wksDefinition);
    openLayersRegisterProjections(lib_default);
  }
 ...
 */
//# sourceMappingURL=@vaadin_flow-frontend_vaadin-map_mapConnector__js.js.map
